...
import ShareEmailPlan from './use-cases/share-email-plan';
...
import govNotifyGateway from "./gateways/govNotify-gateway";
...
// Switching to gov notify instead of sms client
...
const sharePlan = withLogging(
  new SharePlan({ planGateway, smsGateway: govNotifyGateway, logger }),
  logger
);
const shareEmailPlan = withLogging(
    new ShareEmailPlan({ planGateway, emailGateway: govNotifyGateway, logger }),
    logger
);
