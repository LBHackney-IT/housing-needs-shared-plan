...
describe('Plan', () => {
  ...
  describe('update', () => {
    it('updates the plan', () => {
      const plan = new Plan({ numbers: ['123'] });
      plan.update({
        numbers: ['456']
      });
      expect(plan.numbers).toEqual(['456']);
    });

    it('ignores controlled properties', () => {
      const plan = new Plan({ lastName: 'jones', numbers: ['123'] });
      plan.update({ lastName: 'thomas' });
      expect(plan.lastName).toEqual('jones');
    });
  });
});
