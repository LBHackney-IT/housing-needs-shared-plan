...
describe('Share plan', () => {
  ...
  it('updates the token in the database after sharing it', async () => {
    planGateway.get = jest.fn(() => ({
      customerTokens: [
        { token: 'abc', createdDate: new Date(2020, 3, 5), shared: false }
      ]
    }));
  ...
  it('shares a correct link that has not been shared with the customer via sms', async () => {
    ...
    planGateway.get = jest.fn(() => ({
      ...
      customerTokens: [
        { token: 'abc', createdDate: new Date(2020, 3, 5), shared: false },
        { token: 'ooo', createdDate: new Date(2020, 3, 5), shared: true }
      ]
    }));
    ...
    const expectedSmsRequest = {
      ...
      message: expect.stringContaining(
        `${process.env.NEXT_PUBLIC_URL}/customer/plans/${planId}?token=abc`
      ),
