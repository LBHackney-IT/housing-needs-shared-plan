...
describe('UpdateAction', () => {
  ...
  beforeEach(() => {
    usecase.getPlan.mockImplementation(() => Promise.resolve(expectedPlan));
  });

  describe('execute', () => {
    it('throws PlanNotFoundError if a plan is not found', () => {
      usecase.getPlan.mockImplementation(() => Promise.resolve(null));
      expect(
        usecase.execute({
    ...
    describe('missing actions', () => {
      ...
      it('throws ActionNotFoundError error if no goal is set', () => {
        usecase.getPlan.mockImplementation(() =>
          Promise.resolve(
            new Plan({
              id: 'd6BQiWGGhOrF8mFdPp4T',
              created: '2020-05-04T00:00:00+0000',
